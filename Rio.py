...
class Rio(Entorno):
    ...
    def mostrar_acciones(self,ventana,font,negro):
            for index,frase in enumerate(self.frases):
                msg_accion = font.render(frase, True, negro)
                ventana.blit(msg_accion, [1150, 130+20*(index+1)])
    def ejecutar(self, agente):
        ...
        while not finalizado:
            ...
            msg_accion = font.render("Acciones Agente:", True, negro)
            ventana.blit(msg_accion, [1000, 110])
            msg_accion = font.render("V=verdugos, P=pacificos", True, negro)
            ventana.blit(msg_accion, [1000, 130])
            self.mostrar_acciones(ventana,font,negro)
            ...
            if 700 + 119 > cursor[0] > 700 and 45 + 36 > cursor[1] > 45:
                ...
                if pygame.mouse.get_pressed() == (1, 0, 0):
                    ...
                    agente.tecnica = "profundidad"
                    agente.set_estado_inicial(estado)
            ...
            if con_agente:
                ...
                self.frases=[]
                self.habla.say("P significa pacificos y V significa verdugos")
                # realiza las diferencias entre estados para obtener las acciones a realizar
                for indice in range(1,len(acciones)):
                    pacificos=abs(acciones[indice-1][0]-acciones[indice][0])
                    verdugos=abs(acciones[indice-1][1]-acciones[indice][1])
                    frase= "Bote: "
                    if pacificos!=0:
                        # agrega la frase X pacificos
                        frase+=f" {pacificos} P"
                    if verdugos!=0:
                        # agrega la frase X verdugos
                        frase+=f" {verdugos} V"
                    self.habla.say(frase)
                    self.frases.append(frase)
                    print([pacificos,verdugos])
                # muestra en la ventana las acciones obtenidas 
                self.mostrar_acciones(ventana,font,negro)
                pygame.display.update()
